...
const PROPERTY_TYPES = [
  "Self Contain", "Single Room", "Mini Flat", "1 Bedroom Flat",
  "2 Bedroom Flat", "3 Bedroom Flat", "Shared Room", "Studio Apartment",
];
...
export default function AddListingPage() {
  ...
  }, [formData.price, formData.cautionFee, formData.legalFee, formData.serviceCharge, agencyFeeAmount]);
  ...
  function handleImageChange(e) {
    ...
    setErrors((prev) => ({ ...prev, media: "", general: "" }));
  }
  ...
  function validate() {
    const e = {};
    if (imageFiles.length === 0 && !videoFile) e.media = "Add at least one photo or video";
    if (!formData.type)               e.type    = "Pick a property type";
    ...
    if (!formData.caption.trim())     e.caption = "Say a bit about the place";
    else if (formData.caption.trim().length < 10) e.caption = "A little more detail please";
    return e;
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        setUploadProgress(`Sending photo ${i + 1} of ${imageFiles.length}…`);
      ...
      const mapsUrl = "https://www.google.com/maps/search/?api=1&query=" +
        encodeURIComponent((formData.address.trim() ? formData.address.trim() + ", " : "") + mapBase);

      const title = `${formData.type} in ${selectedLocation?.label || formData.location}`;
      ...
      await createListing({
        ...
        agencyFeePercent: formData.agencyFeePercent ? Number(formData.agencyFeePercent) : 0,
  ...
  return (
    <main className="alp">
      <div className="alp__header">
        <span className="alp__step-label">New listing</span>
      </div>
      ...
      <div className="alp__body">
        ...
        {submitted && (
          <div className="alp__success">
            ...
            Listing posted! Taking you there now…
          </div>
        )}

        {errors.general && <div className="alp__error-banner">{errors.general}</div>}

        <form onSubmit={handleSubmit} noValidate className="alp__compose">

          {/* ── Attach: the first move, like sharing a photo in chat ── */}
          <div className="alp__field" data-field="media">
            <div className="alp__media-row">
              <label className="alp__media-add" htmlFor="photo-input">
                <span className="alp__media-add-icon">+</span>
                <span className="alp__media-add-text">Photos</span>
              </label>

              {imagePreviews.map((src, i) => (
                <div key={i} className="alp__media-item">
              ...
              {videoFile && (
                <div className="alp__media-item alp__media-item--video">
                  {videoThumbnailPreview ? <img src={videoThumbnailPreview} alt="Video thumbnail" /> : <video src={videoPreview} muted />}
                  <span className="alp__media-play">▶</span>
                  <button type="button" className="alp__photo-remove" onClick={removeVideo}>✕</button>
                </div>
              )}
            </div>
            <input
            ...
            {!videoFile && (
              <label className="alp__media-video-link" htmlFor="video-input">
                + Add a walkthrough video <span className="alp__optional">optional</span>
              </label>
            )}
            <input
              id="video-input"
          ...
          {/* ── Rent: the first number anyone asks for ── */}
          <div className="alp__field" data-field="price">
            <label>Annual rent</label>
            <div className="alp__price-input alp__price-input--hero">
              <span className="alp__price-prefix">₦</span>
              <input
                type="number"
                name="price"
                placeholder="300,000"
          ...
          {/* ── Agent fee, paired with rent, total done for them ── */}
          <div className="alp__field">
            <label>Agent fee <span className="alp__optional">optional — 0 if none</span></label>
            <div className="alp__percent-input">
              <input
                ...
                inputMode="numeric"
              />
              <span className="alp__percent-suffix">%</span>
            </div>
          </div>
          ...
          {Number(formData.price) > 0 && (
            <div className="alp__total alp__total--top">
              ...
              {agencyFeeAmount > 0 && (
                <span className="alp__hint">Rent ₦{Number(formData.price).toLocaleString()} + agent fee ₦{agencyFeeAmount.toLocaleString()}</span>
              )}
            </div>
          )}

          {/* ── Type: one tap ── */}
          <div className="alp__field" data-field="type">
            ...
            <div className="alp__chip-row">
              {PROPERTY_TYPES.map((t) => (
                <button
                  type="button"
                  key={t}
                  className={"alp__chip" + (formData.type === t ? " alp__chip--active" : "")}
          ...
          {/* ── Area ── */}
          <div className="alp__field" data-field="location">
            <label>Area</label>
            ...
            {selectedLocation && <span className="alp__hint">{selectedLocation.hint}</span>}
            {errors.location && <span className="alp__err">{errors.location}</span>}
          </div>

          {/* ── Contact ── */}
          <div className="alp__field" data-field="contact">
          ...
          {/* ── Caption: everything else, in one breath ── */}
          <div className="alp__field" data-field="caption">
            ...
            <textarea
              ...
              onChange={handleChange}
            />
            {errors.caption && <span className="alp__err">{errors.caption}</span>}
          </div>

          {/* ── Everything structured, tucked away ── */}
          <div className="alp__more">
            ...
            >
              ...
              <span className="alp__optional">optional — beds, fees, furnishing…</span>
              <span className={"alp__chevron" + (showMore ? " alp__chevron--open" : "")}>⌄</span>
            </button>

            {showMore && (
              <div className="alp__more-body">
                <div className="alp__field">
                  <label>Street address <span className="alp__optional">optional — helps tenants find it on the map</span></label>
                  <input
                    type="text"
                    name="address"
                ...
                <div className="alp__row">
                  <div className="alp__field">
                    <label>Availability <span className="alp__optional">optional</span></label>
                  ...
                  <div className="alp__field">
                    <label>Furnishing <span className="alp__optional">optional</span></label>
                ...
                <div className="alp__field">
                  <label>Payment terms <span className="alp__optional">optional</span></label>
                  <input
                    type="text"
                    name="paymentTerms"
                ...
                <div className="alp__fees-label">
                  Extra move-in fees <span className="alp__optional">enter 0 to skip</span>
                </div>
                <div className="alp__row">
                  <div className="alp__field">
          ...
          {uploadProgress && <div className="alp__progress">{uploadProgress}</div>}

          <div className="alp__nav">
            <button type="submit" className="alp__btn-next" disabled={saving}>
              {saving ? (uploadProgress || "Posting…") : "Post listing"}
            </button>
          </div>
